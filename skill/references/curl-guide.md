...
```bash
curl -s https://www.llmfill.com/v1/account/whoami -H "X-Auth-Token: aif_你的令牌"
...
curl -s -OJ https://www.llmfill.com/v1/documents/b_xxx/result \
  -H "X-Auth-Token: aif_你的令牌"          # -OJ 按响应文件名保存
...
curl -s -X POST https://www.llmfill.com/v1/knowledge-bases \
  -H "X-Auth-Token: aif_你的令牌" -H "Content-Type: application/json" \
...
curl -s -X POST https://www.llmfill.com/v1/knowledge-bases/kb-xxx/documents \
  -H "X-Auth-Token: aif_你的令牌" -F "file=@手册.pdf"
...
```bash
# 知识库列表 / 文档列表
curl -s https://www.llmfill.com/v1/knowledge-bases -H "X-Auth-Token: aif_你的令牌"
curl -s https://www.llmfill.com/v1/knowledge-bases/kb-xxx/documents -H "X-Auth-Token: aif_你的令牌"

# 批次历史列表
curl -s https://www.llmfill.com/v1/documents -H "X-Auth-Token: aif_你的令牌"

# 接口自动发现（拉聚合 OpenAPI schema）
curl -s https://www.llmfill.com/v1/openapi

# 删除批次（仅 completed/failed/partial 可删；删除同时清掉服务端结果文件，下载要趁早）
curl -s -X DELETE https://www.llmfill.com/v1/documents/batches/b_xxx -H "X-Auth-Token: aif_你的令牌"
```
